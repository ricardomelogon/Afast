<script>
	$(document).ready(function(e) {
		...
		$('#datepicker').on("changeDate", function() {
			...
			$('#docenteloader').load('docentes/docentes.lista.data.php',{ data: data});
		});
		...
		$('.CarregaDocentesCurso').click(function(e) {
			e.preventDefault();
			var id= $(this).attr('id');
			$('#docenteloader').load('docentes/docentes.lista.curso.php',{ id: id});
		});
		
		$('#CarregaTodosDocentes').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('docentes/docentes.lista.todos.php');
		});
		
		$('#CadastrarDocente').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('docentes/docentes.cadastrar.php');
		});
		
		$('#GerenciarCursos').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('curso/curso.lista.php');
		});

		$('#GerenciarOcorrencias').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('ocorrencias/ocorrencias.lista.php');
		});

		$('#GerenciarAfastamentos').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('afastamento/afastamento.listar.php');
		});

		$('#InserirAfastamento').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('afastamento/afastamento.cadastrar.embutido.php');
		});
		
		$('#InserirAfastamentoSerie').click(function(e) {
			e.preventDefault();
			$('#docenteloader').load('afastamento/afastamento.cadastrar.serie.php');
		});
...
$Data = getdate ();
        ...
        <section class="col-md-12">
			<button class="btn btn-primary col-md-12" id="CadastrarDocente"
		...
		<section class="col-md-12">
			<button class="btn btn-ocorrencia col-md-12" id="GerenciarOcorrencias"
				type="button">Gerenciar Ocorrências</button>
		</section>
	...
	<section class="col-md-8 sidebar-content" id="docenteloader">
		...
		<div class="col-md-12 well">
			<h2>Gerenciar Docentes Cadastrados</h2>
            
            <section ><h3>Selecione uma opção ao lado</h3></section>
		</div>
	</section>
	<!-- Paginas carregadas aqui -->
</section>
